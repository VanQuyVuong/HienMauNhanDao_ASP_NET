...
export default function DanhSachChoKham() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      try {
        ...
        const fallbackRes = await donDangKyNvytService.getAll(
          0,
          FETCH_CHUNK,
          keyword,
        );
        ...
        list = raw.map((d) => ({
          ...
          maTNV: d.maTNV,
          tenTinhNguyenVien:
            d.tinhNguyenVien?.hoTen ||
            d.tinhNguyenVien?.hoVaTen ||
            "TNV Hiến Máu",
          ngaySinh: d.tinhNguyenVien?.ngaySinh
            ? String(d.tinhNguyenVien.ngaySinh)
            : "---",
      ...
      if (keyword) {
        ...
        list = list.filter(
          (d) =>
            ...
            (d.tenTinhNguyenVien &&
              d.tenTinhNguyenVien.toLowerCase().includes(kw)) ||
  ...
  const displayedList = useMemo(() => {
    return filteredAll.filter((item) => {
      const matchesBlood =
        bloodFilter === "ALL" ||
        String(item.nhomMau || "").includes(bloodFilter);
      return matchesBlood;
    });
  ...
  const handleGoKham = (don) => {
    navigate("/bac-si/kham-lam-sang", {
      state: { maDon: don.maDon, donData: don },
    });
  };

  const registeredByLeTanCount = filteredAll.filter(
    (d) => d.maNhanVien || d.maNV,
  ).length;
  ...
  return (
    <div className="space-y-6">
      {toast && (
        <div
          className={`fixed top-6 right-6 z-[100] px-5 py-3.5 rounded-2xl shadow-2xl text-white text-xs font-black flex items-center gap-2.5 transition-all animate-bounce
        ...
        <div className="relative z-10 flex flex-col lg:flex-row lg:items-center justify-between gap-6">
          <div>
            ...
            <p className="text-rose-100 text-xs md:text-sm mt-1.5 max-w-2xl font-medium leading-relaxed">
              Tiếp nhận tình nguyện viên đã check-in qua bàn Lễ tân, tiến hành
              đo 4 chỉ số sinh hiệu (Huyết áp, nhịp tim, cân nặng, nhiệt độ) và
              quyết định cấp phép hiến máu
            </p>
          </div>
          ...
          <div className="grid grid-cols-3 gap-3">
            <div className="bg-white/20 backdrop-blur-md border border-white/30 rounded-2xl p-3.5 px-4 text-center shadow-sm">
              <p className="text-[10px] font-black uppercase text-rose-100 tracking-wider">
                Chờ Khám Ngay
              </p>
            ...
            <div className="bg-white/20 backdrop-blur-md border border-white/30 rounded-2xl p-3.5 px-4 text-center shadow-sm">
              ...
              <p className="text-2xl font-black text-white mt-1">
                {filteredAll.length - registeredByLeTanCount}
              </p>
            </div>
          </div>
        </div>
      ...
      <div className="bg-white border border-rose-100 rounded-3xl shadow-sm overflow-hidden">
        <div className="p-4 bg-rose-50/50 border-b border-rose-100 flex flex-col sm:flex-row sm:items-center justify-between gap-2">
          ...
          <span className="text-[11px] font-bold text-rose-700 bg-rose-100/70 px-3 py-1 rounded-xl border border-rose-200 shadow-2xs">
            💡 Bấm nút{" "}
            <b className="text-rose-700 font-black">🩺 Gọi Khám Lâm Sàng</b> để
            đo 4 chỉ số sinh hiệu
          </span>
        </div>
            ...
            <thead>
              <tr className="bg-rose-50/30 border-b border-rose-100">
                {[
                  ...
                  <th
                    key={h}
                    className="text-left px-5 py-3.5 text-[11px] font-black uppercase text-slate-400 tracking-wider whitespace-nowrap"
                  >
                    {h}
                  </th>
                ))}
              ...
              {loading ? (
                ...
                <tr>
                  <td
                    ...
                    className="text-center py-12 text-slate-400 font-bold"
                  >
                    Hiện không có tình nguyện viên nào chờ khám.
                  </td>
                </tr>
              ) : (
                dons.map((don) => {
                    ...
                    >
                      ...
                      <td className="px-5 py-4">
                        <p className="font-extrabold text-slate-800 text-sm">
                          {don.tenTinhNguyenVien || "---"}
                        </p>
                        <p className="text-[10px] text-slate-400 font-medium">
                          Giới tính: {don.gioiTinh || "---"} | Ngày sinh:{" "}
                          {don.ngaySinh || "---"}
                        </p>
                      </td>
                      <td className="px-5 py-4">
                      ...
                      <td className="px-5 py-4">
                        <button
                          ...
                          className="h-10 px-4 bg-gradient-to-r from-rose-600 via-red-600 to-rose-700 hover:from-rose-700 hover:to-red-700 text-white rounded-xl font-black text-xs flex items-center gap-1.5 shadow-md shadow-rose-500/25 active:scale-95 transition-all whitespace-nowrap"
                        >
        ...
        {totalPages > 1 && (
          <div className="flex justify-between items-center px-5 py-3.5 border-t border-rose-100 bg-rose-50/40">
            <span className="text-xs font-bold text-slate-500">
              Trang {page + 1} / {totalPages} ({displayedList.length} ca chờ)
            </span>
            <div className="flex gap-2">
              <button
                onClick={() => setPage((p) => Math.max(0, p - 1))}
                disabled={page === 0}
                className="w-8 h-8 rounded-xl border border-slate-200 flex justify-center items-center hover:bg-slate-100 disabled:opacity-40 font-bold"
              >
