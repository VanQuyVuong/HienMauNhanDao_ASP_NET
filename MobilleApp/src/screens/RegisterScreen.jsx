...
export default function RegisterScreen({ route, navigation }) {
  ...
  const handleRegisterOrSendOtp = async () => {
    ...
    try {
      if (!emailVerified) {
        // Bước 1: Gửi OTP trước khi cho phép đăng ký
        await authService.sendOtp(email.trim());
        Alert.alert('Thành công', 'Mã OTP đã được gửi thành công. Vui lòng kiểm tra email của bạn.', [
          {
            text: 'Nhập OTP',
        ...
        // Bước 2: Thực hiện đăng ký khi đã xác thực OTP thành công
      ...
      const msg = error.response?.data?.message || error.message || 'Lỗi trong quá trình xử lý';
      Alert.alert('Lỗi', msg);
    } finally {
      setLoading(false);
    }
  ...
  return (
    <View style={styles.container}>
      ...
      <TouchableOpacity style={styles.button} onPress={handleRegisterOrSendOtp} disabled={loading}>
        {loading ? (
...
const styles = StyleSheet.create({
  ...
  title: { fontSize: 28, fontWeight: 'bold', color: '#b7102a', textAlign: 'center', marginBottom: 8 },
